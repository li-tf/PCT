#!/usr/bin/env python3
"""Run one D1 projection with four physical silicon tracking planes."""
...
def expected_output_names(config: dict) -> list[str]:
    planes = list(config["reference_planes"]) + list(config["tracker_planes"])
    return [f"{item['name']}.root" for item in planes]


def validate_config(config: dict, angle: int, protons: int) -> None:
    if int(config["projections"]) != 720 or not 0 <= angle < 720:
        raise ValueError("D1 requires angle in [0, 719] and exactly 720 projections")
    ...
    if config["world_material"] != "Air" or config["tracker_material"] != "Silicon":
        raise ValueError("D1 requires Air world and Silicon trackers")
    if len(config["tracker_planes"]) != 4 or len(config["reference_planes"]) != 2:
        raise ValueError("D1 requires four tracker planes and two reference planes")
    if not math.isclose(float(config["tracker_size_mm"][2]), 0.2, abs_tol=1e-12):
        raise ValueError("tracker thickness must be 0.2 mm")
    tracker_z = [float(item["z_mm"]) for item in config["tracker_planes"]]
    reference_z = [float(item["z_mm"]) for item in config["reference_planes"]]
    if tracker_z != [-160.0, -130.0, 130.0, 160.0] or reference_z != [-110.0, 110.0]:
        raise ValueError("unexpected D1 plane locations")
    if len(config["insert_radii_mm"]) != 25:
        raise ValueError("D1 requires the 25-insert results0716 phantom")
    focus_distance = abs(float(config["focus_z_mm"]) - float(config["source_z_mm"]))
    field = [float(value) * abs(float(config["focus_z_mm"])) / focus_distance
             for value in config["source_size_mm"][:2]]
    if not np.allclose(field, [250.0, 2.0], atol=1e-9):
        raise ValueError(f"unexpected isocenter field: {field}")
    names = [item["name"] for item in config["reference_planes"] + config["tracker_planes"]]
...
def add_phantom(sim: gate.Simulation, config: dict, angle_index: int) -> None:
    ...
    for insert_id, radial_mm in enumerate(config["insert_radii_mm"]):
        phi = math.radians(float(config["insert_angle_step_deg"]) * insert_id)
        ...
        insert.translation = [float(radial_mm) * math.cos(phi) * mm,
                              float(radial_mm) * math.sin(phi) * mm, 0]
...
def add_phase_actor(sim: gate.Simulation, volume_name: str, actor_name: str,
                    output_dir: Path, attributes: list[str]) -> None:
    ...
    actor.output_filename = str(output_dir / f"{actor_name}.root")
...
def add_reference_plane(sim: gate.Simulation, config: dict, item: dict,
    ...
    volume.size = [float(v) * mm for v in config["reference_size_mm"]]
    volume.translation = [0, 0, float(item["z_mm"]) * mm]
    volume.material = "Air"
    add_phase_actor(sim, volume.name, name, output_dir, config["output_attributes"])


def add_tracker_plane(sim: gate.Simulation, config: dict, item: dict,
                      output_dir: Path) -> None:
    mm = gate.g4_units.mm
    name = str(item["name"])
    volume = sim.add_volume("Box", f"Volume{name}")
    volume.size = [float(v) * mm for v in config["tracker_size_mm"]]
    volume.translation = [0, 0, float(item["z_mm"]) * mm]
    volume.material = str(config["tracker_material"])
    volume.set_max_step_size(float(config["tracker_max_step_mm"]) * mm)
    add_phase_actor(sim, volume.name, name, output_dir, config["output_attributes"])
...
def build_simulation(config: dict, angle: int, protons: int, output_dir: Path,
    ...
    for item in config["tracker_planes"]:
        add_tracker_plane(sim, config, item, output_dir)
...
def inspect_root(path: Path, tree_name: str) -> dict:
    ...
    return {
        ...
        "finite_sample_rows": sample_stop,
    }
...
def main() -> None:
    ...
    record = {
        ...
        "angle_index": args.angle, "angle_degrees": args.angle * 0.5,
        "protons_per_projection": protons, "random_seed": seed,
        "started_at": started.isoformat(timespec="seconds"), "host": platform.node(),
    ...
    try:
        ...
        if args.build_only:
            ...
            metadata_path.write_text(json.dumps(record, indent=2) + "\n", encoding="utf-8")
            print(f"D1 build-only PASS for angle {args.angle:03d}")
            return
        record["status"] = "running"
        metadata_path.write_text(json.dumps(record, indent=2) + "\n", encoding="utf-8")
        print(f"Starting D1 angle {args.angle:03d}, protons={protons:,}, seed={seed}", flush=True)
        sim.run()
        summaries = {}
        for filename in expected_output_names(config):
            path = output_dir / filename
            if not path.is_file():
                raise RuntimeError(f"missing output: {filename}")
            summaries[filename] = inspect_root(path, path.stem)
        ...
        print(f"Completed D1 angle {args.angle:03d} in {record['elapsed_seconds']:.1f} s", flush=True)
    except Exception:
